...
sitophilus <- data.frame(
    "extract" = factor(rep(c("Leaf", "Branch", "Seed", "Control"),
                           each = 10),
                       levels = c("Control", "Leaf", "Branch", "Seed")),
...
mvobs <- sitophilus %>%
    ...
    setNames(c("Extract",
               "\\makecell[c]{Sample \\\\[-.8mm] mean}",
               "\\makecell[c]{Sample \\\\[-.8mm] variance}"))

cap <-
    "Sample mean and sample variance for the \\textit{Sitophilus zeamais} data."
print.xtable(xtable(mvobs, caption = cap),
             size = "small",
...
# Models
models <- list("CMP" = m1cmp,
               "GCT" = m1gct,
               "GPo" = m1gpo,
               "PTw" = m1ptw)

# Coef tab
aux <- lapply(models[1:3], function(x) {
    tab <- summary(x)@coef[, 1:2]
    tab <- tab[c(2:nrow(tab), 1), ]
    rbind(tab, c(Inf, Inf))
})
aux$PTw <- do.call(rbind, summary(m1ptw)[[1]])[, 1:2]

# Compact display
ctab <- lapply(aux, function(x) {
    rownames(x) <- 1:6
    out <- sprintf("$%s (%s)$",
                   sprintf("%.3f", x[, 1]),
                   sprintf("%.3f", x[, 2]))
...
ctab <- do.call(cbind, ctab)
rownames(ctab) <- c("$\\beta_0$",
                    sprintf(
                        "$\\tau_{\\text{%s}}$",
                        tolower(levels(sitophilus$extract)[-1])),
                    "Disp.",
                    "Power")

lls <- sapply(models, logLik)
aics <- sapply(models, AIC)
gofs <- t(vapply(list(lls, aics), sprintf, fmt = "$%.3f$",
                 FUN.VALUE = character(4)))
rownames(gofs) <- c("LogLik", "AIC")

## ---- ctab-models
cap <- "Parameter estimates and standard errors for the model strategies."
print.xtable(xtable(rbind(ctab, gofs), caption = cap,
                    align = c("lrrrr")),
             add.to.row = list(
                 list(6),
